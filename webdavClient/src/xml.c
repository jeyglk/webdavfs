 ...
 */
 ...
 /** Modified by Jeremy Mathevet <jeyg.lk@gmail.com>
 */
#include <ctype.h>
#include <malloc.h>
#include <stdarg.h>
 ...
 * [PRIVATE]
 * 
 * Skips the HTTP response
 */
static void xml_skip_http(struct xml_parser* parser) {
	xml_parser_info(parser, "http");
 ...
 * [PRIVATE]
 * 
 * Skips the XML header
 */
static void xml_skip_xml_header(struct xml_parser* parser) {
	xml_parser_info(parser, "http");

	if ('<' == parser->buffer[parser->position]) {
		if ('?' == parser->buffer[parser->position + 1]) {
			parser->position++;
			while ('>' != parser->buffer[parser->position]) {
				parser->position++;
			}
...
static struct xml_string* xml_parse_tag_end(struct xml_parser* parser) {
	...
	/* 'Multistatus' special tag
	 * Skip xmlns
	 */
	if ('m' == parser->buffer[parser->position + 2]) {
		xml_parser_consume(parser, 13);
		length = length + 13;
	...
	/* '<resourcetype/>' tag special case
	 */
	if (('<' == xml_parser_peek(parser, CURRENT_CHARACTER)) 
		&& ('r' == xml_parser_peek(parser, CURRENT_CHARACTER + 3))
			&& ('/' == xml_parser_peek(parser, CURRENT_CHARACTER + 15))) {

		parser->position = parser->position + 17;
	}
...
static struct xml_string* xml_parse_content(struct xml_parser* parser) {
	...
	while (start + length < parser->length) {
		...
		if ( ('<' == current) && ('c' == xml_parser_peek(parser, CURRENT_CHARACTER + 3)) ) {
			xml_parser_consume(parser, 1);
			length++;
		} else  if ('<' == current) {
			break;
...
static struct xml_node* xml_parse_node(struct xml_parser* parser) {
	...
	/* 'Collection' tag special case
	 */
	if (('<' == xml_parser_peek(parser, CURRENT_CHARACTER)) 
		&& ('c' == xml_parser_peek(parser, CURRENT_CHARACTER + 3))) {

		content = xml_parse_content(parser);
	
		if (!content) {
				xml_parser_error(parser, 0, "xml_parse_node::content_collection");
				goto exit_failure;
		}
	}
...
struct xml_document* xml_parse_document(uint8_t* buffer, size_t length) {
	 ...
	 */
	xml_skip_http(&parser);
	xml_skip_xml_header(&parser);
